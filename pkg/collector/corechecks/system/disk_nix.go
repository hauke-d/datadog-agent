...
func (c *DiskCheck) sendDiskMetrics(sender aggregator.Sender, ioCounter, lastIoCounter disk.IOCountersStat, tags []string) {
	sender.Count(fmt.Sprintf(diskMetric, "read_time"), float64(incrementWithOverflow(ioCounter.ReadTime, lastIoCounter.ReadTime))/1000, "", tags)
	sender.Count(fmt.Sprintf(diskMetric, "write_time"), float64(incrementWithOverflow(ioCounter.WriteTime, lastIoCounter.WriteTime))/1000, "", tags)
